...
const categoryColors = {
  "Governança Digital": "text-blue-400 bg-blue-500/10 border-blue-500/20",
  "Inteligência Artificial": "text-purple-400 bg-purple-500/10 border-purple-500/20",
  "Estratégia": "text-indigo-400 bg-indigo-500/10 border-indigo-500/20",
  "Inovação": "text-cyan-400 bg-cyan-500/10 border-cyan-500/20",
  "Digital Business": "text-emerald-400 bg-emerald-500/10 border-emerald-500/20",
};
...
function PostCard({ post, index }) {
  const colorClass = categoryColors[post.category] || "text-blue-400 bg-blue-500/10 border-blue-500/20";
  ...
  return (
    <motion.article
      ...
      className="group flex flex-col rounded-2xl border border-white/[0.06] bg-white/[0.02] hover:border-white/10 hover:bg-white/[0.04] transition-all duration-500 overflow-hidden"
    >
      ...
      <div className="flex flex-col flex-1 p-7">
        ...
        <h3 className="text-lg font-semibold text-white leading-snug mb-3 group-hover:text-blue-100 transition-colors">
          {post.title}
        </h3>

        <p className="text-white/40 text-sm font-light leading-relaxed flex-1">
          {post.excerpt}
        </p>

        <div className="mt-6 flex items-center justify-between">
          <span className="text-white/20 text-xs">{formattedDate}</span>
          <span className="flex items-center gap-1 text-blue-400 text-sm font-medium opacity-0 group-hover:opacity-100 translate-x-2 group-hover:translate-x-0 transition-all duration-300">
...
export default function BlogSection() {
  ...
  return (
    <section id="blog" className="relative py-28 md:py-36 bg-[#070B14]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          <div>
            ...
            <p className="text-white/40 mt-3 text-base font-light max-w-lg">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-xs font-medium tracking-wide transition-all duration-300 ${
                  activeCategory === cat
                    ...
                    : "bg-white/[0.04] text-white/40 border border-white/[0.06] hover:text-white/70 hover:border-white/10"
                }`}
              >
                {cat}
              </button>
        ...
        {loading ? (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1, 2, 3].map((i) => (
              <div key={i} className="rounded-2xl border border-white/[0.06] bg-white/[0.02] h-80 animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
