...
export default function TestimonialsSection() {
  return (
    <section className="relative py-28 md:py-36 bg-[#0A0F1C]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {testimonials.map((testimonial, i) => (
            ...
            >
              <div className="relative rounded-2xl p-8 md:p-10 bg-white/[0.02] border border-white/[0.06] hover:border-white/10 transition-all duration-500">
                ...
                <div className="flex items-center gap-4">
                  <div className="w-11 h-11 rounded-full bg-gradient-to-br from-blue-500/20 to-indigo-500/20 border border-blue-500/10 flex items-center justify-center">
                    <span className="text-sm font-semibold text-blue-300">{testimonial.initials}</span>
                  </div>
                  <div>
                    <div className="text-white font-medium text-sm">{testimonial.name}</div>
                    <div className="text-white/30 text-xs mt-0.5">{testimonial.role}</div>
                  </div>
                </div>
              </div>
            </motion.div>
